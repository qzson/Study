...
# 자원 성능, 무엇이 효율적인지.
'''
LSTM_scale vs simpleRNN vs GRU (같은 튜닝 값, 많은 데이터량)
...
* LSTM_scale vs GRU
  ...
  정확도 : LSTM > GRU  |  자원대비 효율성  : LSTM < GRU
= scale 기준, 정확도는 LSTM 우세. BUT, 총 파라미터 수는 LSTM 이 더 많다. (약 0.5 배 차이)
...
model.add(GRU(900, input_length=3, input_dim=1))
model.add(Dense(450))
model.add(Dense(230))
model.add(Dense(100))
model.add(Dense(1))
